...
- Transport: For `HTTP`, the tokens are sent in the `Authorization` header as `Bearer` tokens. For `gRPC`, the tokens are sent in the `metadata` as `authorization` tokens.
- Key Management:
