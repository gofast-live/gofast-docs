...
This choice can be either made using **GoFast CLI** or by editing `docker-compose.yml` file. Possible options are `SQLite`, `Turso with Embedded Replicas`, `PostgreSQL (local)` and `PostgreSQL (remote)`.
