...
- for `lemon`, set `LEMON_API_KEY` and `LEMON_PRICE_ID`. (WIP)
...
The payments have a separate file for HTTP routes: `/http/route_payment.go`.
...
4. Copy the `Price ID` and set it as the `STRIPE_PRICE_ID`.
